...
public class EtamExtractor extends ProductExtractorBase implements
IEntityExtractor{
	...
	private final Pattern descriptionPattern = Pattern.compile(
			"<meta name=\"description\" content=\"([\\w\\W]*?)\"?>", Pattern.MULTILINE);


	public int extractProduct() throws Throwable {
		Pattern pattern = Pattern.compile("http://www.etam.com.cn/[\\w\\W]{1,1024}");
		int cnt = 0;
		db = new DBManager();
	...
	private void extractDetailPage(CrawlerTask task) throws Throwable {
		...
		//获取序列码
		String tempStr;
		String regx = "<div class=\"sku\">款号:([\\w\\W]*?)</div>";
...
//			tempStr = new String(tempStr.getBytes("gbk"),"utf-8");
		...
		//获取价格信息
		regx = "<span class=\"price\">([\\w\\W]*?)</span>";
		...
		//获取图片url
		regx = "<img[\\w\\W]*?class=\"normal-pic\"[\\w\\W]*?src=\"([\\w\\W]*?)\"[\\w\\W]*?alt=\"\"[\\w\\W]*?title=\"\"[\\w\\W]*?/>";
