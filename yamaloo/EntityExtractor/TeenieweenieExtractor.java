...
public class TeenieweenieExtractor extends ProductExtractorBase implements
IEntityExtractor{
	private final Pattern namePattern = Pattern.compile(
			"<!--//goods_name-->[\\w\\W]*?id=\"ctl00_ContentPlaceHolder1_StyleContent_DispStyleName\">([\\w\\W]*?)</span>", Pattern.MULTILINE);

	public int extractProduct() throws Throwable {
		Pattern pattern = Pattern.compile("http://www.teenieweenie.com.cn/Style/StyleDetail.aspx[\\w\\W]*");
		int cnt = 0;
		db = new DBManager();
...
//		description = new String(description.getBytes("gbk"),"utf-8");
		...
		//获取序列码
		String tempStr;
		String regx = "<!--//style_number-->[\\w\\W]*?</td>[\\w\\W]*?>([\\w\\W]*?)</td>";
...
//			tempStr = new String(tempStr.getBytes("gbk"),"utf-8");
		...
		//获取价格信息
		regx = "<!--//goods_price-->[\\w\\W]*?</td>[\\w\\W]*?Price\">([\\w\\W]*?)</span>";
